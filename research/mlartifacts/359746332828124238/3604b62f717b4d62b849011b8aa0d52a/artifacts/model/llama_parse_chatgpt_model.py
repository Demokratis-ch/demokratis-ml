...
class LlamaParseMarkdownParser(PythonModel):
    ...
    def nodes_to_json(self, nodes):
        ...
        parsed_nodes, num_input_tokens, num_output_tokens = loop.run_until_complete(asyncio.gather(*[parse_node_with_semaphore(node) for node in nodes]))
    ...
    def predict(self, context, model_input: List[str]) -> Dict[str, Any]:
        ...
        parsed_files = []
        markdown_texts = []
        num_input_tokens_tot = 0
        num_output_tokens_tot = 0
        for file_path in model_input:
            ...
            json_schema, num_input_tokens, num_input_tokens = self.nodes_to_json(nodes)
            parsed_files.append(json_schema)
            num_input_tokens_tot += num_input_tokens
            num_output_tokens_tot += num_output_tokens
