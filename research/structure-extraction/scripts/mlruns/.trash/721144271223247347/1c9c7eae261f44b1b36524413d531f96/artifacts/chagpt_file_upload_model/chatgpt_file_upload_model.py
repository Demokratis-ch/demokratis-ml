...
class ChatgptFileParsing(PythonModel):
    ...
    async def parse_file(self, file_path: str) -> Dict[str, str]:
        # Upload the file to OpenAI
    ...
    async def predict_async(self, context, model_input: List[str]) -> List[Dict]:
        tasks = [parse_file(filename) for filename in model_input]
    ...
    def predict(self, context, model_input: List[str]) -> List[Dict]:
        # tasks = [parse_file(filename) for filename in model_input]
        # results = asyncio.run(await asyncio.gather(*tasks))

        parsed_dicts, num_input_tokens, num_output_tokens = asyncio.run(self.predict_async(context, model_input))            
