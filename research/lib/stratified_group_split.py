...
class MultilabelStratifiedGroupShuffleSplit(MultilabelStratifiedShuffleSplit):
    def split(
        ...
        """Perform a stratified group shuffle split on a DataFrame of samples.

        To achieve the grouping, the following steps are taken:
...
class SplitResult:
    ...
    def __post_init__(self) -> None:
        """Ensure that the shapes of the data are consistent."""
...
def one_simple_split(
    ...
    """Use the configured ``splitter`` to perform a single split on the data.

    :returns: (train, test) splits as :class:`SplitResult` instances.
    """
...
def _group_split(
    ...
    for groups_train_index, groups_test_index in original_splitter(
        ...
        samples_train_index = _convert_group_indices_to_sample_indices(per_group_sample_indices, groups_train_index)
        samples_test_index = _convert_group_indices_to_sample_indices(per_group_sample_indices, groups_test_index)
...
def _convert_group_indices_to_sample_indices(
    per_group_sample_indices: Mapping[int, tuple[int, ...]],
    group_indices: np.ndarray,
) -> np.ndarray:
    """Unroll the group indices generated by :func:`_create_per_group_indices_and_labels` back into sample indices."""
